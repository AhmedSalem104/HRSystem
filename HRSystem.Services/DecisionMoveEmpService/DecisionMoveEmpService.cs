...
{
    public class DecisionMoveEmpService : IDecisionMoveEmpService
    {
        #region Fields
        readonly PayrollEntities _context;
        #endregion
        #region Constructors
        public DecisionMoveEmpService(PayrollEntities context)
        {
            _context = context;
        ...
        #region Methods
        public List<DecisionMoveEmpDTO> GetAll()
        {

            var List = (from DM in _context.DecisionMoveEmps
                                   join E in _context.Employees on DM.EmpId equals E.EmpID



                                   select new DecisionMoveEmpDTO()
                                   {
                                       DecisionMoveEmpID = DM.DecisionMoveEmpID,
                                       DateH = DM.DateH,
                                       EmpName = E.EmpName,
                                       EnclosureCount = DM.EnclosureCount,
        ...
        public DecisionMoveEmp GetDecisionMoveEmpById(int? id)
        {
            return _context.DecisionMoveEmps.Where(c => c.DecisionMoveEmpID == id).FirstOrDefault();
        }

        public int? GetDecisionMoveEmpCode()
        {
            //return _context.Warning.Max(a => a.WarnCode) + 1;
           
            var CodeNotNull = _context.DecisionMoveEmps.Count();
            if (CodeNotNull != 0)
            {
                var Code = _context.DecisionMoveEmps.Max(a => a.DecisionMoveEmpID);
        ...
        public bool Insert(DecisionMoveEmp DecisionMoveEmp)
        {
            try
            {

                _context.DecisionMoveEmps.Add(DecisionMoveEmp);
        ...
        public bool Update(DecisionMoveEmp DecisionMoveEmp)
        {
            try
            {


                if (DecisionMoveEmp == null)
                {
                    throw new ArgumentNullException("DecisionMoveEmp");
                }
                _context.Entry(DecisionMoveEmp).State = EntityState.Detached;
                _context.Entry(DecisionMoveEmp).State = EntityState.Modified;
                _context.SaveChanges();
                return true;
        ...
        public bool Delete(int id)
        {
            try
            {
                ...
                var driver = _context.DecisionMoveEmps.FirstOrDefault(m => m.DecisionMoveEmpID == id);
        ...
        public List<GetDecisionMoveEmpForPrint_Result> SearchListForPrint()
        {

            var list = _context.GetDecisionMoveEmpForPrint().ToList();

            return list;

        }

        public List<GetDecisionMoveEmpForPrint_Result> GetDecisionMoveEmpForPrint(int[] ids)
        {
            var List = SearchListForPrint().Where(a => ids.Contains(a.DecisionMoveEmpID)).ToList();
            return List;
        }
        ...
        public int GetMaxId()
        {
            var count = _context.DecisionMoveEmps.ToList();
            var id = 0;
            if (count.Count() != 0)
            {
                id = _context.DecisionMoveEmps.Max(a => a.DecisionMoveEmpID);
                return id;
            }
            return id;
        }
