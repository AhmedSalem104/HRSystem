...
{
    public class DecisionChangeJobService : IDecisionChangeJobService
    {
        #region Fields
        readonly PayrollEntities _context;
        #endregion
        #region Constructors
        public DecisionChangeJobService(PayrollEntities context)
        {
            _context = context;
        ...
        #region Methods
        public List<DecisionChangeJobDTO> GetAll()
        {

            var List = (from DCJ in _context.DecisionChangeJobs
                                   join E in _context.Employees on DCJ.EmpId equals E.EmpID



                                   select new DecisionChangeJobDTO()
                                   {
                                       DecisionChangeJobID = DCJ.DecisionChangeJobID,
                                       DateH = DCJ.DateH,
                                       EmpName = E.EmpName,
                                       EnclosureCount = DCJ.EnclosureCount,
        ...
        public DecisionChangeJob GetDecisionChangeJobById(int? id)
        {
            return _context.DecisionChangeJobs.Where(c => c.DecisionChangeJobID == id).FirstOrDefault();
        }

        public int? GetMaxDecisionChangeJobCode()
        {
            //return _context.Warning.Max(a => a.WarnCode) + 1;
           
            var CodeNotNull = _context.DecisionChangeJobs.Count();
            if (CodeNotNull != 0)
            {
                var Code = _context.DecisionChangeJobs.Max(a => a.DecisionChangeJobID);
        ...
        public bool Insert(DecisionChangeJob DecisionChangeJob)
        {
            try
            {

                _context.DecisionChangeJobs.Add(DecisionChangeJob);
        ...
        public bool Update(DecisionChangeJob DecisionChangeJob)
        {
            try
            {


                if (DecisionChangeJob == null)
                {
                    throw new ArgumentNullException("DecisionChangeJob");
                }
                _context.Entry(DecisionChangeJob).State = EntityState.Detached;
                _context.Entry(DecisionChangeJob).State = EntityState.Modified;
                _context.SaveChanges();
                return true;
        ...
        public bool Delete(int id)
        {
            try
            {
                ...
                var driver = _context.DecisionChangeJobs.FirstOrDefault(m => m.DecisionChangeJobID == id);
        ...
        public List<GetDecisionChangeJobForPrint_Result> SearchListForPrint()
        {
          var list = _context.GetDecisionChangeJobForPrint().ToList();
        ...
        public List<GetDecisionChangeJobForPrint_Result> GetDecisionChangeJobForPrint(int[] ids)
        {
            var List = SearchListForPrint().Where(a => ids.Contains(a.DecisionChangeJobID)).ToList();
            return List;
        }
